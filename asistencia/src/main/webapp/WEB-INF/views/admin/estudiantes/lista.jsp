...
<head>
    ...
    <style>
        ...
        .card-header {
            background-color: #3498db !important; /* Color primario de Bootstrap */
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="${pageContext.request.contextPath}/admin/estudiantes">
    ...
    <div class="row">
        <div class="col-md-12 p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="${pageContext.request.contextPath}/admin/estudiantes/nuevo" class="btn btn-primary btn-lg">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <c:forEach items="${alumnos}" var="alumno">
                                <tr>
                                    <td><span class="badge bg-secondary">${alumno.id}</span></td>
                                    <td><strong>${alumno.nombre}</strong></td>
                                    <td>${alumno.apellido}</td>
                                    <td>${alumno.seccion.bachillerato.nombre}</td>
                                    <td><span class="badge bg-info">${alumno.seccion.nombre}</span></td>
                                    <td>${alumno.especialidad.nombre}</td>
                                    <td>
                                            ${alumno.responsable.nombre} ${alumno.responsable.apellido}
                                        <br><small class="text-muted">${alumno.responsable.telefono}</small>
                                    </td>
                                    <td>
                                        <div class="btn-group" role="group">
                                            <a href="${pageContext.request.contextPath}/admin/estudiantes/editar/${alumno.id}"
                                               class="btn btn-sm btn-warning" title="Editar">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                            <a href="${pageContext.request.contextPath}/admin/estudiantes/eliminar/${alumno.id}"
                                               class="btn btn-sm btn-danger"
                                               onclick="return confirm('¿Está seguro de eliminar a ${alumno.nombre} ${alumno.apellido}?')"
                                               title="Eliminar">
                        ...
                        <c:if test="${empty alumnos}">
                            <div class="alert alert-info text-center">
                                ...
                                <p class="mt-2">No hay estudiantes registrados. Haz clic en "Nuevo Estudiante" para agregar uno.</p>
                            </div>
                        </c:if>
                    </div>
                </div>
                <div class="card-footer">
                    <small class="text-muted">Total de estudiantes: <strong>${alumnos.size()}</strong></small>
                </div>
            </div>
        </div>
    </div>
